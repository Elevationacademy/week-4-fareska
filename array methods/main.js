...
let emailAndCompany = users1.map(ec => {return {email:ec.email, company:ec.company.name}} )
console.log(emailAndCompany)

/////////// Exercise 2

const zipNum= users1.filter(u => u.address.zipcode.slice(0,1) === "5")

console.log(zipNum)

/////////// Exercise 3

const zipNum1= users1.filter(u => u.address.zipcode.slice(0,1) === "5").map(u => u.id)

console.log(zipNum1)

/////////// exercise 4
const onlyNames = users1.map(u => u.name)
const onlyNames1 = onlyNames.filter(n => n.slice(0,1) === "C")
console.log(onlyNames1)




/*
///////////// ES6 Array Methods- Material///////////////////
...
let goodPeople = people.filter(p => (p.goodPerformance))
console.log(goodPeople)
const bonus = (person) => { if (person.goodPerformance){person.salary += 300}}
people.forEach(bonus)
/////////// map

let messagesFromDad = ["HI HONEY", "HOW WAS SCHOOL??", "DID YOU EAT TODAY?", "I CAN'T FIND THE REMOTE CONTROL"]

let lowerCase = messagesFromDad.map (lC => lC.toLowerCase())
